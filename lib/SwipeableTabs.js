...
var SwipeableTabs = function (_React$Component) {
  ...
  (0, _createClass3.default)(SwipeableTabs, [{
    ...
    value: function resetScroll(index) {
      ...
      var prevIndex = this.props.index;
      requestAnimationFrame(function () {
        var _window = window,
            scrollY = _window.scrollY,
            scrollX = _window.scrollX;

        _this2._scrollPosition[prevIndex] = scrollY;
        ...
        window.scrollTo(scrollX, _this2._scrollPosition[index] || 0);
      });
    }
  }, {
    key: 'onSwitching',
    value: function onSwitching(index, mode) {
      ...
      requestAnimationFrame(function () {
        if (_this3._tabHeader) {
          _this3._tabHeader.syncGuide(index, mode);
        }
        if (mode === 'move' && !_this3.isAligned) {
          _this3.isAligned = true;
          var scrollTop = window.scrollY;

          for (var i = 0; i < _this3._tabs.length; i++) {
            if (i !== _this3.props.index) {
              _this3._tabs[i].translateY(scrollTop - (_this3._scrollPosition[i] || 0));
            }
          }
        }
        if (mode === 'move') {
          _this3.unclampHeight(index);
        } else {
          setTimeout(function () {
            requestAnimationFrame(function () {
              _this3.clampHeightExcept(index);
            });
            // parseFloat will auto ignore the 's'
          }, parseFloat(_this3.props.springConfig.duration));
        }
      });
    }
  }, {
    key: 'unclampHeight',
    value: function unclampHeight(index) {
      var tab1 = this._tabs[Math.ceil(index)];
      var tab2 = this._tabs[Math.floor(index)];
    ...
    key: 'clampHeightExcept',
    value: function clampHeightExcept(index) {
